...
public:
	...
	GameServerInformations(uint16 id, int8 status, int8 completion, bool isSelectable, int8 charactersCount, int64 date) : id(id), status(status), completion(completion), isSelectable(isSelectable), charactersCount(charactersCount), date(date)
	{
	}
